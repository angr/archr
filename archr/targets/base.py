...
class Target(ABC):
    ...
    def __init__(
        ...
        *,
        ...
        if self.target_arch is None:
            log.debug("target architecture not specified, using `x86_64` by default")
            self.target_arch = "x86_64"
        if self.target_os is None:
            log.debug("target OS not specified, using `linux` by default")
    ...
    def run_command(
        ...
        **kwargs,  # for us  # for subclasses
        ...
        log.debug("Running command: '%s'", "' '".join(command_args))
